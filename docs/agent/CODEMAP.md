...
| `app/services/session_store.py` | `HostSession`, `LabEvent`, `SessionStore`, `session_store`; iframe kind aliases (`iframe_client_kind_from_event_body`) |
| `app/services/host_session_auth.py` | Cookie + bearer gates; `HOST_SESSION_COOKIE_MAX_AGE`; `request_user_agent` |
| `app/services/host_tokens.py` | `mint_host_access_token`, `verify_host_access_token`, `host_session_id_from_claims`, `apply_auth0_token_set` |
...
| `app/static/js/src/iframe-client-kind.js` | Canonical `iframe_client_kind` writers for both embed tabs |
...
| `tests/` | Pytest: token state, freeze, SESSION_DEAD 409, drop vs expire, host JWT claim and `iframe_client_kind` cutovers |
...
Relevant fields: Auth0 tokens + claims; host JWT + `jti` + claim `host_session_id` (legacy `hsid`); Looker four tokens + TTLs; flags `freeze_token_refresh`, `force_user_agent_mismatch`, `session_reference_dropped`, `looker_session_revoked`, `host_session_revoked`; per-iframe SDK/postmessage started/expired; `user_agent` (login); `events`; `refresh_markers`.
...
`lab.js` constants: `EMBED_SDK_TAB = "embed-sdk"`, `POSTMESSAGE_TAB = "postmessage"`. Client events: `iframe_client_kind` (`embed_sdk` / `raw_postmessage`). Controls: `#toggle-freeze`, `#toggle-user-agent-mismatch`, `#btn-drop-session-reference`, `#btn-end-looker`.
