...
| --- | --- | --- | --- |
| Embed SDK | `embed-sdk-tab.js` | `startEmbedSdkTab` | acquire + generate callbacks. `syncCookielessRemainingTtls` rewrites SDK-cached TTLs to remaining seconds. `proactivelyGenerateTokens` calls generate and pushes `session:tokens` at 180s remaining. |
