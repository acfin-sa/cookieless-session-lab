...
- The split between Looker's autonomous `session:tokens:request` and the host-written generate callback. Do not echo acquire TTLs on a later ask. The SDK generates only when `Date.now() > generateTokensTime` (original TTL minus 120s). Looker's ask lands on that instant, so the iframe dies near page time 8:39 while the session reference remains. Keep `syncCookielessRemainingTtls` and `proactivelyGenerateTokens`.
